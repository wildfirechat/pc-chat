...
class WfcImpl {
    ...
    async muteGroupMembers(groupId, isMute, memberIds = [], lines = [0], notifyContent, successCB, failCB) {
        let payload = notifyContent ? notifyContent.encode() : '';
        let notifyContentStr = JSON.stringify(payload);
        proto.muteGroupMember(groupId, isMute, memberIds, lines, notifyContentStr, successCB, failCB);
    }

    async allowGroupMembers(groupId, isAllow, memberIds = [], lines = [0], notifyContent, successCB, failCB) {
        let payload = notifyContent ? notifyContent.encode() : '';
        let notifyContentStr = JSON.stringify(payload);
        proto.allowGroupMembers(groupId, isAllow, memberIds, lines, notifyContentStr, successCB, failCB);
    }
    ...
    getConversationFileRecords(conversation, beforeUid, count, successCB, failCB) {
        proto.getConversationFiles(conversation, beforeUid, count, successCB, failCB);
    }


    getMyFileRecords(beforeUid, count, successCB, failCB) {
        proto.getMyFiles(beforeUid, count, successCB, failCB);
    }

    deleteFileRecord(beforeUid, count, successCB, failCB) {
        proto.deleteFileRecord(beforeUid, count, successCB, failCB);
    }
