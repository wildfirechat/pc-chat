// 基本消息类型
export const ContentType_Unknown = 0;
export const ContentType_Text = 1;
export const ContentType_Voice = 2;
export const ContentType_Image = 3;
export const ContentType_Location = 4;
export const ContentType_File = 5;
export const ContentType_Video = 6;
export const ContentType_Sticker = 7;
export const ContentType_ImageText = 8;

// 提醒消息
export const ContentType_Tip_Notification = 90;

export const ContenType_CreateGroupMember_Notification = 104;
export const ContenType_AddGroupMember_Notification = 105;
export const ContenType_KickOffGroupMember_Notification = 106;
export const ContenType_QuitGroup_Notification = 107;
export const ContenType_DismissGroup_Notification = 108;
export const ContenType_TransferGroupOwner_Notification = 109;
export const ContenType_ChangeGroupName_Notification = 110;
export const ContenType_ModifyGroupAlias_Notification = 111;
export const ContenType_ChangeGroupPortrait_Notification = 112;